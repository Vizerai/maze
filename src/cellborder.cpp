...
void LineBorder::DrawBorder(const Cairo::RefPtr<Cairo::Context> &cr) const {
  cr->move_to(x1_*40 + 50, y1_*40 + 50);
  cr->line_to(x2_*40 + 50, y2_*40 + 50);
}
...
void ArcBorder::DrawBorder(const Cairo::RefPtr<Cairo::Context> &cr) const {
  cr->move_to(cx_, cy_);
  cr->line_to(cx_, cy_);
}